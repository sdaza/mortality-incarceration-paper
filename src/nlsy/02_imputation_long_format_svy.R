...
table(org[stratum == 1, cluster])
table(org[stratum == 2, cluster])
...
for (i in 1:10) { # number of imputation
      ...
      t <- limp[.imp == i]
      ds <- svydesign(id=~cluster, prob=~wt, strata=~stratum, data = t, nest = TRUE)
...
for (i in 1:10) { # number of imputation
      ...
      ds <- svydesign(id=~cluster, prob=~nwt, strata=~stratum, data = t, nest = TRUE)
