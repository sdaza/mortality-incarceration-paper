...
# paths, functions and files

# my laptop
library(here)
setwd(here("05Research/sdaza"))
load("output/rdata/psid/long_selection_covariates_year_unit_13.Rdata")
...
table(ldat[, .(year, nrprison)], useNA = "ifany")
summary(ldat$start)
...
table(temp$select95, useNA = "ifany")
...
table(temp$tprison, useNA = "ifany")
table(temp[, .(prison = any(tprison == 1), died = any(died == 1)), pid][, .(prison, died)], useNA = "ifany")
...
# race
prop.table(table(temp[start == 0, fracei], useNA = "ifany"))
...
table(temp$tprison95, useNA = "ifany")
table(temp$nrprison, useNA = "ifany")

temp[, temp_prison := pmax(tprison95, nrprison, na.rm = TRUE)]
summary(temp$temp_prison)
table(temp[, temp_prison], useNA = "ifany")
...
temp_imp <- parmice(data = ex, predictorMatrix = mat , method = meth,
              maxit = 20, n.core = 10, n.imp.core = 1)
...
table(tdat$gprison, useNA = "ifany")
...
temp <- svycoxph( Surv(start, stop, died) ~ gprison + male
                                  + cagei + fracei + linca + ieduc + cluster(pid), design = ds)

if ( file.exists("models_wt_msm.Rdata") ) { load("models_wt_msm.Rdata") }
coeff_wt_msm <- c(coeff_wt_msm, list(coefficients(temp)))
vcov_wt_msm <- c(vcov_wt_msm, list(vcov(temp)))
save(coeff_wt_msm, vcov_wt_msm, file = "models_wt_msm.Rdata")
remove(temp)
...
ds <- svydesign(id = ~cluster, weights = ~nwt, strata = ~stratum, data = t, nest = TRUE)
temp <- svycoxph( Surv(start, stop, died) ~ gprison + male
                                  + cagei + fracei + linca + ieduc + dghealth + cluster(pid), design = ds)

if ( file.exists("models_h_wt_msm.Rdata") ) { load("models_h_wt_msm.Rdata") }
coeff_h_wt_msm <- c(coeff_h_wt_msm, list(coefficients(temp)))
vcov_h_wt_msm <- c(vcov_h_wt_msm, list(vcov(temp)))
save(coeff_h_wt_msm, vcov_h_wt_msm, file = "models_h_wt_msm.Rdata")
remove(temp)
...
if ( file.exists("models_uwt.Rdata") ) { load("models_uwt.Rdata") }
coeff_uwt <- c(coeff_uwt, list(coefficients(temp)))
vcov_uwt <- c(vcov_uwt, list(vcov(temp)))
save(coeff_uwt, vcov_uwt, file = "models_uwt.Rdata")
remove(temp)

temp <- coxph( Surv(start, stop, died) ~ gprison + male + cagei + fracei +
                           + linca + ieduc + dghealth + cluster(pid), data = t)

if ( file.exists("models_h_uwt.Rdata") ) { load("models_h_uwt.Rdata") }
coeff_h_uwt <- c(coeff_h_uwt, list(coefficients(temp)))
vcov_h_uwt <- c(vcov_h_uwt, list(vcov(temp)))
save(coeff_h_uwt, vcov_h_uwt, file = "models_h_uwt.Rdata")
remove(temp)
...
temp <- coxph( Surv(start, stop, died) ~ gprison + male
                                  + cagei + fracei + linca + ieduc + cluster(pid), data = t, weights = t$nwt)

if ( file.exists("models_uwt_msm.Rdata") ) { load("models_uwt_msm.Rdata") }
coeff_uwt_msm <- c(coeff_uwt_msm, list(coefficients(temp)))
vcov_uwt_msm <- c(vcov_uwt_msm, list(vcov(temp)))
save(coeff_uwt_msm, vcov_uwt_msm, file = "models_uwt_msm.Rdata")
remove(temp)
...
t[, nwt :=  w1$ipw.weights * w2$ipw.weights]
temp <- coxph( Surv(start, stop, died) ~ gprison + male
                                  + cagei + fracei + linca + ieduc + dghealth + cluster(pid), data = t, weights = t$nwt)

if ( file.exists("models_h_uwt_msm.Rdata") ) { load("models_h_uwt_msm.Rdata") }
coeff_h_uwt_msm <- c(coeff_h_uwt_msm, list(coefficients(temp)))
vcov_h_uwt_msm <- c(vcov_h_uwt_msm, list(vcov(temp)))
save(coeff_h_uwt_msm, vcov_h_uwt_msm, file = "models_h_uwt_msm.Rdata")
remove(temp)
